...
2. **Issue** — create a GitHub issue describing what you are about to do. Note the issue number.
3. **Explore** — list files, read existing code. Understand the structure and style before changing anything.
4. **Plan** — briefly state what files you will create or modify and why.
5. **Implement** — write clean, idiomatic Python. Match the project's existing style.
6. **Test** — run the code. Read every line of output. Fix failures before moving on.
7. **Commit** — make atomic commits with concise conventional commit messages (feat:, fix:, refactor:, test:, docs:).
8. **PR** — push and open a pull request that closes the issue(s) you created in step 2.

## Rules
- **Always create an issue before writing any code.** Every PR must reference at least one issue via closes_issues.
