...
from src.agents.jules.system_prompt import JULES_SYSTEM_PROMPT
...
from src.tools.sandbox import SANDBOX_TOOL_DEFINITIONS, SandboxToolKit
from src.tools.skills import READ_SKILL
from src.tools.web_search import TOOL_DEFINITION as WEB_SEARCH
from src.tools.web_search import web_search

class Jules(CodeAgent):
    """Jules - a Java Spring Boot coding agent with a sandboxed Docker workspace.

    Must be used as an async context manager so the sandbox container
    is started and stopped cleanly:

        async with Jules(...) as jules:
            result = await jules.work(question=..., ...)
    """

    model_config = ConfigDict(arbitrary_types_allowed=True)

    GITHUB_NICKNAME: ClassVar[str] = "Nexus-Jules"
    sandbox_config: SandboxConfig = SPRING_BOOT_JAVA_21
    ...
    async def __aenter__(self) -> "Jules":
        """Start Jules sandbox resources and initialize tool kits."""
    ...
    async def _ensure_fork(self, token: str, upstream_repo: str) -> str:
        """Check if Jules' fork exists; create it if not. Returns the fork name."""
    ...
    @track(tags=["exec", "jules"], step_type="llm", llm_provider=LLMProvider.OPENAI)
    async def step(self, current_turn_ctx: List[ChatCompletionMessageParam]) -> BaseAgentStepResult:
        if self._sandbox is None:
            raise RuntimeError("Jules must be used as an async context manager (async with Jules(...) as jules:)")
        ...
        if stream_result.finish_reason is None:
            raise ValueError("Jules stream completion missing finish_reason.")
    ...
    def last_report_current_process(self, current_turn_ctx: List[ChatCompletionMessageParam]) -> str:
        for msg in reversed(current_turn_ctx):
            if isinstance(msg, dict) and msg.get("role") == "assistant":
                ...
                if content:
                    return content
        return "Jules reached the maximum number of attempts without completing the task."
    ...
    def create(
        ...
        return cls(
            ...
            base_url=base_url,
            api_key=api_key,
            system_prompt=JULES_SYSTEM_PROMPT,
