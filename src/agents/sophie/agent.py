...
class Sophie(CodeAgent):
    ...
    async def __aenter__(self) -> "Sophie":
        """Start Sophie sandbox resources and initialize tool kits.

        Returns:
            The initialized Sophie instance.
        """
    ...
    @track(tags=["exec", "sophie"], step_type="llm", llm_provider=LLMProvider.OPENAI)
    async def step(self, current_turn_ctx: List[ChatCompletionMessageParam]) -> BaseAgentStepResult:
        ...
        if stream_result.finish_reason is None:
            raise ValueError("Sophie stream completion missing finish_reason.")
